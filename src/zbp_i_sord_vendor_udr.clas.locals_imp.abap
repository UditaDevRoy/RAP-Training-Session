...
*** Declaration
    TYPES: BEGIN OF ty_buffer_ord.
             INCLUDE TYPE zsord_vendor_UDR AS data.
...
*** Get last saved sales order
        SELECT MAX( sord ) FROM zsord_vendor_UDR INTO @DATA(lv_max_sord).
  ...
  METHOD update.
    ...
    LOOP AT entities ASSIGNING FIELD-SYMBOL(<lfs_new_data>).
      ...
      SELECT SINGLE * FROM zsord_vendor_UDR WHERE sord = @<lfs_new_data>-Sord
...
*** Get already saved data
    LOOP AT keys ASSIGNING FIELD-SYMBOL(<lfs_keys>).
      SELECT * FROM zsord_vendor_UDR WHERE sord = @<lfs_keys>-Sord INTO TABLE @DATA(lt_result_tmp) .
...
*** Enable/Disable rows based on Overall Status
    ...
    LOOP AT keys ASSIGNING FIELD-SYMBOL(<lfs_keys>).
      SELECT SINGLE * FROM zsord_vendor_UDR WHERE sord = @<lfs_keys>-Sord INTO @DATA(ls_result_tmp) .
...
CLASS lsc_ZI_SORD_VENDOR_UDR DEFINITION INHERITING FROM cl_abap_behavior_saver.
  PROTECTED SECTION.
...
CLASS lsc_ZI_SORD_VENDOR_UDR IMPLEMENTATION.
